...
{/* TODO: your Request Parameters table below doesn't list the model field yet.
    Add a `model` row (allowed values: v6, v6-pro, v7, v7-pro) so this maps to a real param. */}
Pass your choice with the `model` parameter (`v6`, `v6-pro`, `v7`, or `v7-pro`). Streaming is available on the `pro` models for paid users.
...
| `streaming` | Boolean | Optional | false | If `true`, stream audio generation in real time |
