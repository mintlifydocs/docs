...
| `lyrics`       | `String`     | Optional | Original lyrics of song. Max 3000 characters.                   |
...
| `lyrics_timestamps`  | Boolean | Optional | If true, generates lyric timestamps for the output track |
...
* Extend conversion details — one webhook per generated version, so `num_outputs` webhooks (default `2`)
* Lyrics with timestamp data — one webhook per version, sent only when `lyrics_timestamps` is `true`
* Album Cover Image — a single webhook, sent only when `generate_album_cover` is `true`
