...
| `image_url`         | `String`     | Optional | URL to the image file. Required if `image_file` is not provided.                    |
| `prompt`            | `String`     | Optional | Additional text to guide the song generation (max 1000 characters).                                 |
| `lyrics`            | `String`     | Optional | Custom lyrics to include in the generated audio (max 5000 characters).                             |
...
>**Note:** Either image_file or image_url must be provided.
...
# This endpoint expects multipart/form-data, so send the fields as form parts
response = requests.post(
    url, headers=headers, files={k: (None, str(v)) for k, v in data.items()}
)
...
* **422 Unprocessable Entity**: Neither `image_file` nor `image_url` provided.
