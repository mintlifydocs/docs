...
| `lyrics`      | `String`     | Optional     | Lyrics to guide remix or vocal generation. Max 5000 characters.           |
| `gender`      | `String`     | Optional     | Vocal tone. One of: `male`, `female`, `neutral`.                          |
| `generate_album_cover` | Boolean | Optional | If `true`, generate an album cover for the generated audio. Defaults to `false`. |
| `lyrics_timestamps`  | Boolean | Optional | If `true`, generates lyric timestamps for the output track. Defaults to `false`. |
...
* 1 Album Cover Image — sent only when `generate_album_cover` is `true`
