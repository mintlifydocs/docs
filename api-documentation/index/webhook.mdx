...
Webhooks allow Lalals to notify your application the moment a task is completed. You’ll get all the necessary data — including the audio URL — without needing to constantly poll our API.
...
2. Lalals will **send a POST request** to that URL when the task is completed.
...
When you initiate a conversion request, Lalals will immediately respond with a **preliminary response** that includes:
...
Set it once, and let Lalals notify you when the magic happens. 🎶
