
<AccordionGroup>

  <Accordion title="What are the prompt guidelines for AI Music Generation?">
    <p>To get the best results, follow these rules:</p>
    <ul>
      <li>Keep prompts descriptive and simple</li>
      <li><strong>Prompt limit:</strong> 1000 characters</li>
      <li><strong>Lyrics limit:</strong> 5000 characters</li>
    </ul>

    <p>You can describe song sections like this:</p>

    <pre>
[Intro – soft piano and light synths]
[Chorus – repeat with more intensity]
[Bridge – add soft strings or a subtle beat drop]
    </pre>

    <p><strong>Note:</strong> Do not include any artist names.</p>
  </Accordion>

  <Accordion title="How do I cancel a subscription?">
    <ol>
      <li>Go to the API Dashboard</li>
      <li>Open Plans</li>
      <li>Click on Free</li>
    </ol>
    <p>Your plan will be downgraded immediately.</p>
  </Accordion>

  <Accordion title="How do I get a Voice ID?">
    <p>Use the following endpoints:</p>
    <ul>
      <li><code>[GET /endpoint/searchVoices](/api-documentation/endpoint/searchVoices)</code></li>
      <li><code>[GET /endpoint/getAllVoices](/api-documentation/endpoint/getAllVoices)</code></li>
    </ul>
    <p>The response includes the required <code>voice_id</code>.</p>
  </Accordion>

 <Accordion title="How does the credit system work?">
  <p>Plans do not include credits.</p>
  <p>However, the cost per API call is much lower in higher-tier plans.</p>

  <p><strong>Example (Music AI):</strong></p>
  <ul>
    <li><span>Pro Plan: &#36;0.035 per song</span></li>
    <li><span>Plus Plan: &#36;0.05 per song</span></li>
    <li><span>Pay as You Go: &#36;0.10 per song</span></li>
  </ul>
<p>If you are on the Pro plan, you first pay a &#36;69 base charge and top up credits. Credits are then deducted at &#36;0.035 per song, whereas on Pay as You Go, each song deducts &#36;0.10 per song. </p>
</Accordion>


<Accordion title="Which Endpoints send WAV Files?"> <p> WAV files are currently sent only for the following endpoints: </p> <ul> <li>Music AI</li> <li>Remix</li> <li>Extend</li> <li>Inpaint</li> <li>Sing Over Instrumental</li> </ul> </Accordion>

<Accordion title="How can I contact support?">
  <p>
    If you need help, please email us at <strong>api@musicgpt.com</strong> with a detailed description of your issue.
    If applicable, include relevant information such as your <code>task_id</code>, API request, or any error messages to help us investigate more quickly.
  </p>

  <p>
    <strong>Before you send anything, redact your credentials.</strong> Remove the
    <code>Authorization</code> header, API Keys, and any other sensitive data from requests,
    responses, and logs. Support never needs your API Key to investigate an issue.
  </p>

  <p>
    Our team typically responds within <strong>24 hours</strong>, and often much sooner. We're committed to providing fast, reliable, and high-quality support to help you get back on track.
  </p>
</Accordion>


</AccordionGroup>


