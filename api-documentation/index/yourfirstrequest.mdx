...
-d '{
  ...
  "webhook_url": "https://webhook.yourwebsite.com"
}'
```
...
> Webhook receivers must use HTTPS; `http://` URLs are not accepted.
...
`audio_url` is a signed AWS URL that is valid for 1 day. Download the file within that
window, or fetch a fresh URL from [get conversion by id](/api-documentation/endpoint/getById).
